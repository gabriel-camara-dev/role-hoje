...
import { GetAdminAuthActivityUseCase } from './get-admin-auth-activity';
...
let sut: GetAdminAuthActivityUseCase;

describe('Get Admin Auth Activity', () => {
  beforeEach(() => {
    usersRepository = new InMemoryOndeHojeUsersRepository();
    dashboardRepository = new InMemoryAdminDashboardRepository();
    sut = new GetAdminAuthActivityUseCase(dashboardRepository, usersRepository);
  });

  it('returns the auth activity for an admin', async () => {
    ...
    expect((result as { value: { authActivity: unknown } }).value.authActivity).toBe(dashboardRepository.authActivity);
  });
