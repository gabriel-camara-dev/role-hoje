...
import { GetAdminAbuseReportUseCase } from './get-admin-abuse-report';
...
let sut: GetAdminAbuseReportUseCase;

describe('Get Admin Abuse Report', () => {
  beforeEach(() => {
    usersRepository = new InMemoryOndeHojeUsersRepository();
    dashboardRepository = new InMemoryAdminDashboardRepository();
    sut = new GetAdminAbuseReportUseCase(dashboardRepository, usersRepository);
  });

  it('returns the abuse report for an admin', async () => {
    ...
    expect((result as { value: { abuseReport: unknown } }).value.abuseReport).toBe(dashboardRepository.abuseReport);
  });
